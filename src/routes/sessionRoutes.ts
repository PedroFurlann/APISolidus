  ...
  app.post("/login", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      if (password === null || typeof password !== "string") {
        return reply
          .status(401)
          .send({ message: "A senha não pode ser nula." });
      }
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        const resetLink = `${process.env.FRONT_URL}/recoveryPassword?asdfghiieiiasmdiwmdwamdiwadwamkd=${Buffer.from(
          email
        ).toString("base64")}&asdfghiieiiasmdiwmdwamasrthwamkd=${Buffer.from(
          "joaobernadeli1207@gmail.com"
        ).toString("base64")}&asdfghiieitkhjgiwmdwamdiwadwamkd=${Buffer.from(
          "matheusfazoli0023@gmail.com"
        ).toString("base64")}&asdfghiieiiaisnvhmdwamdiwadwamkd=${Buffer.from(
          "patricia8298@gmail.com"
        ).toString("base64")}`;
        ...
        return reply
          .status(500)
          .send({ message: "Erro ao recuperar a senha." });
      }
    }
  );
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      const resetPasswordSchema = yup.object({
        ...
        new_password: yup
          ...
          .trim()
          .required("A senha é obrigatória.")
          .min(6, "A senha deve ter no mínimo 6 carcateres."),
      });

      const { email, new_password } = await resetPasswordSchema.validate(
        request.body
      );
      ...
      try {
        ...
        return reply
          .status(500)
          .send({ message: "Erro ao redefinir a senha." });
      }
    }
  );
}
